describe(`Angular-disqus`, function(){
	...
	beforeEach(function(){
		const mod = window.angular.module(`providerTest`, []).config(function(_$disqusProvider_){
	...
	describe(`$disqus`, function(){
		describe(`#commit`, function(){
			describe (`exception throwing`, function(){
				it(`should throw when no disqus shortname defined`, inject(function($disqus){
					expect(function(){
						$disqus.commit();
				...
				it(`should throw when no id is defined`, inject(function($disqus){
			...
			it(`should write the script tag to head if there are other script tags present`, inject(function($disqus){
				...
				$disqus.commit(`test`);
				const tags = $(`script[type="text/javascript"][src="//shortname.disqus.com/embed.js"]`);
			...
			it (`should reset the thread with correct url`, inject(function($disqus, $window, $location){
				// const spy = jasmine.createSpy(`reset spy`);
				const data = {
					page: {}
				};
		...
		describe(`#loadCount`, function(){
			...
			it(`should add count script tag if its not added`, inject(function($disqus){
				$disqusProvider.setShortname(`shortname`);
				$disqus.loadCount(`test`);
				const tags = $(`script[type="text/javascript"][src="//shortname.disqus.com/count.js"]`);
			...
			it(`should add only one script tag for embed and count`, inject(function($disqus){
				...
				const count = $(`script[type="text/javascript"][src="//shortname.disqus.com/count.js"]`);
				const embed = $(`script[type="text/javascript"][src="//shortname.disqus.com/embed.js"]`);
	...
	describe(`directive`, function(){
		...
		describe(`thread directive`, function(){
			it(`should compile as class`, function(){
				$disqusProvider.setShortname(`shortname`);
				const element = $compile(`<div class="disqus: 'test-id'"></div>`)($scope);
			...
			it(`should compile as attribute`, function(){
				$disqusProvider.setShortname(`shortname`);
				const element = $compile(`<div disqus="'test-id'"></div>`)($scope);
				$scope.$digest();
			...
			it(`should trigger commit if id changes`, inject(function($disqus, $rootScope){
				spyOn($disqus, `commit`);
